...
const zoom = (targetScale, center = null, scaleTime = 0) => {
  ...
  const target = {
    scale: targetScale,
    x:
      (coords.x - relativeZoomCenter.x * screen.x) * (targetScale / scale) +
      relativeZoomCenter.x * screen.x,
    y:
      (coords.y - relativeZoomCenter.y * screen.y) * (targetScale / scale) +
      relativeZoomCenter.y * screen.y,
  }
...
const initialize = () => {
  ...
  Drag.on(stage, start, move, end, { highlight: false, type: 'map' })
  Secondary.on(stage, ({ coords, shiftKey }) => {
    if (!Foreground.hasOpenScreen()) {
      ...
      if (selectedUnit) {
        const target = {
          x: Math.floor(
            (coords.x - RenderView.get().coords.x) / (TILE_SIZE * RenderView.get().scale)
          ),
          y: Math.floor(
            (coords.y - RenderView.get().coords.y) / (TILE_SIZE * RenderView.get().scale)
          ),
        }
        ...
        if (shiftKey) {
          Commander.scheduleBehind(
            ...
            MoveTo.create({ unit: selectedUnit, coords: target })
          )
        } else {
  ...
  const handleWheel = ({ delta, position }) => {
    if (!Foreground.hasOpenScreen()) {
      Events.trigger('zoom')
      zoomBy(Math.exp(-ZOOM_FACTOR * delta.y), {
