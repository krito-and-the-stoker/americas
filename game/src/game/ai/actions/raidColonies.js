...
const raiderTargets = relations =>
  State.all(relations, 'colonies')
    .map(colony => ({
...
const distanceSort = base => (a, b) =>
  LA.distance(base.mapCoordinates, b.mapCoordinates) -
  LA.distance(base.mapCoordinates, a.mapCoordinates)
const createRaiderMap = (raiders, targets, threads) => {
  ...
  targets.forEach(target => {
    ...
    raiders.sort(distanceSort(target)).forEach(unit => {
      map
        .find(({ raider }) => raider === unit)
        .targets.push({
  ...
  threads.forEach(thread => {
    ...
    raiders
      .filter(
        raider =>
          LA.distance(thread.mapCoordinates, raider.mapCoordinates) <= 1.25 * thread.radius
      )
...
const interpolateTargets = (targets, weight) => {
  ...
  return LA.multiply(
    1.0 / weightSum,
    targets.reduce(
      (sum, target) => LA.madd(sum, weight(target), target.mapCoordinates),
      LA.vector()
    )
  )
}
...
const create = ({ tribe, state, owner }) => {
  ...
  const unitPlans = Util.range(state.relations[owner.referenceId].raidPlanned)
    .map(() => GetUnit.create({ owner: tribe.owner, coords }))
    .filter(a => !!a)
      ...
      commit: () => {
        ...
        Time.schedule({
          ...
          update: (_, deltaTime) => {
            ...
            createRaiderMap(raiders, targets, threads).forEach(
              ({ raider, targets, threads }) => {
                ...
                // )
                const targetCoords = Util.max(
                  targets,
                  target =>
                    target.attractivity /
                    LA.distance(target.mapCoordinates, raider.mapCoordinates)
                ).mapCoordinates
                const threadCoords = interpolateTargets(threads, thread => thread.attractivity)
                ...
                const targetDirection =
                  (targetCoords && LA.subtract(targetCoords, raider.mapCoordinates)) ||
                  LA.vector()
                const threadDirection =
                  (threadCoords && LA.subtract(threadCoords, raider.mapCoordinates)) ||
                  LA.vector()
                const friendsDirection =
                  (friendsCoords && LA.subtract(friendsCoords, raider.mapCoordinates)) ||
                  LA.vector()
