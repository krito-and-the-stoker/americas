...
const updateTexture = view => {
  const frame = view.unit.expert
    ? view.unit.properties.frame[view.unit.expert] || view.unit.properties.frame.default
    : view.unit.properties.frame.default
  ...
  Record.listen('unit', unit => {
    ...
    view.unsubscribe = [
      ...
      Unit.listen.colonist(unit, colonist =>
        colonist
          ? Colonist.listen.colony(colonist, updateBoundVisibility)
      ...
      Unit.listen.mapCoordinates(
        ...
        Binding.map(
          ...
          tile => tile && Tile.listen.discovered(tile, updateBoundVisibility)
        )
      ),
      Unit.listen.properties(unit, () => {
        updateTexture(view)
      }),
      Unit.listen.expert(unit, () => {
        updateTexture(view)
      }),
      Unit.listen.radius(unit, () => {
        updateRadius(view)
      }),
