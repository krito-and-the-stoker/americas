...
const render = () => {
  ...
  const max = Util.max(
    Util.flatten(allColonies.map(colony => Goods.types.map(good => display(colony, good))))
  )
  const min = Util.min(
    Util.flatten(allColonies.map(colony => Goods.types.map(good => display(colony, good))))
  )
  const color = number => {
    ...
    const fraction = Util.clamp(Math.pow(number > 0 ? number / max : number / min, 0.5))
  ...
  const sum = h('div.total', [
    h(
      'span.name',
      `Colonial Empire (${Util.sum(allColonies.map(colony => colony.colonists.length))})`
    ),
    ...Goods.types.map(good =>
      h(
        'span.storage',
