...
const initialize = () => {
  ...
  const render = unit => {
    if (unit && !Foreground.hasOpenScreen()) {
      ...
      const passengers = unit.passengers.map(passenger =>
        UnitView.html(passenger, PASSENGER_SCALE)
      )
      ...
      const view = h('div.unit-panel.visible', [
        ...
        h(
          ...
          {
            on: {
              click: () => MapView.centerAt(unit.mapCoordinates, 350),
            },
          },
          name
        ),
  ...
  UnitMapView.listen.selectedView(
    Binding.map(
      selectedView => selectedView && selectedView.unit,
      unit =>
        (unit && [
