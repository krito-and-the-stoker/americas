...
const createFromMap = ({ tiles }) => {
  ...
  const zone = tiles
    ...
    .filter(tile => !tile.rumors)
    .filter(tile => tile.domain === 'land')
  let possibleLocations = Util.removeDuplicates(
    Util.range(Math.round(settlementDensity * zone.length)).map(
      () => Util.choose(zone).mapCoordinates
    )
  )

  Util.range(20).forEach(() => {
    possibleLocations = possibleLocations.map(location => {
      const direction = possibleLocations
      ...
      return Util.range(10)
        ...
        .reduce((currentLocation, possibleNewLocation) => {
          return zone.includes(MapEntity.tile(possibleNewLocation))
            ? possibleNewLocation
            : currentLocation
        }, location)
    })
