...
const scheduleRoute = (state, route) => {
  ...
  const goods = route.orders.reduce(
    (s, order) =>
  ...
  Factory.update.display(
    state,
    `Transporting ${goods} from ${route.src.name} to ${route.dest.name}`
  )
    ...
    const update = (...args) => {
      if (state.needsOrders) {
        ...
        if (route) {
          // already there
          if (
            unit.colony === route.src ||
            (route.src.type === 'europe' && Europe.has.unit(unit))
          ) {
