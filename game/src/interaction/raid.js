...
export default (colony, raider) => {
  ...
  const defender =
    Util.max(possibleDefenders, unit => Unit.strength(unit)) || Signal.evaluate.sync(Signal.emit(colony), Colony.chain.defender)
  ...
  if (!defender.properties.combat) {
    ...
    const survivalChance =
      1 - 1 / (2 * colony.buildings.fortifications.level + 1) + 1 / colony.colonists.length
    if (Math.random() < survivalChance) {
      Message.unit.log(
        `A ${raiderName} overcame the defenders of ${colony.name}. The storage has been plundered. A ${defenderName} has barely survived the attack. The colonists in fear.`
      )
    } else {
      Message.unit.log(
        `A ${raiderName} overcame the defenders of ${colony.name}. The storage has been plundered. A ${defenderName} has died in an attempt to defend. The colonists seek revenge.`
      )
      ...
      if (defender.properties.defeated) {
        if (defender.properties.defeated.transfer) {
          defender.properties.defeated.transfer.forEach(good => {
            Storage.transfer(defender.equipment, raider.equipment, {
              good,
            })
          })
        }
  ...
  if (pack) {
    pack.amount = Util.clamp(
      (relativeRaidAmont() * pack.amount * (4 - fortificationLevel)) / 4,
      5,
      100
    )
