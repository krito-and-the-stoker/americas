...
const cheapest = (owner, coords) => {
  const settlements = Record.getAll('settlement')
    ...
    .filter(
      settlement =>
        !lastReserved[settlement.referenceId] ||
        lastReserved[settlement.referenceId] < Time.get().currentTime
    )
  return Util.min(
    settlements,
    settlement => 2 * Util.distance(settlement.mapCoordinates, coords) + cost(settlement)
  )
}
...
const recruit = settlement => {
  ...
  if (settlement.tribe.storage.guns >= 20) {
    Storage.transfer(settlement.tribe.storage, unit.equipment, {
      good: 'guns',
      amount: 20,
    })
  }
