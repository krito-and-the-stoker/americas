...
const tags = {
  spacer: {
    ...
    replace: (content, parse) =>
      h(
        'div.spacer',
        content.split('<->').map(chunk => h('div', parse(chunk)))
      ),
  },
  options: {
    begin: '<options',
    end: '/>',
    replace: (content, parse, context) =>
      h(
        'div.options',
        context.options.map(({ text, action, disabled, margin }) =>
          h(
            'div.option',
            {
    ...
    if (isOpen) {
      el = patch(
        ...
        h(
          ...
          [h('div.background', [h('div.content', [h('div', parse(text))])])]
        )
      )
    } else {
      patch(el, h('!'))
