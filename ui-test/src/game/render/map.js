...
class MapView {
  constructor() {
    ...
    const totalSprites = this.tileStacks.reduce(
      (count, stack) => count + stack.frames.length,
      0
    )
    if (totalSprites > 0) {
  ...
  renderFarm(tile) {
    ...
    if (tile.plowed) {
      ...
      const mod = this.getForestTileModifier(up.plowed, right.plowed, down.plowed, left.plowed)
    ...
    if (center && center.discovered()) {
      ...
      if (left && right && up && down) {
        if (center.hills) {
          ...
          const mod = this.getForestTileModifier(up.hills, right.hills, down.hills, left.hills)
