...
const create = (colony) => {
	...
	const update = (currentTime, deltaTime) => {
		...
		const halfedDeltaFoodWithoutHorses = 2 * deltaTime * PRODUCTION_BASE_FACTOR * Math.floor(0.5 * colony.productionSummary.food)
		...
		const newAmount = Math.min((colony.buildings.stables.level + 1) *
			...
			PRODUCTION_BASE_FACTOR, Math.max(halfedDeltaFoodWithoutHorses, 0))
